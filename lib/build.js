...
function marshalEntry (fileName, entry) {
  ...
  return { description, name, title, tags, createdAt, createdAtDate, body }
}
...
async function spitHtml (templates, entries, tags) {
  ...
  for await (const [tag, entries] of Object.entries(tags)) {
    const html = templates.tag({ config, tag, entries, taglist })
