...
const FIRST_AMONG_EQUALS = ['poetry', 'essay', 'fiction', 'horror', 'sci-fi', 'collection', 'software', 'spooky']
...
async function spitHtml (templates, entries, tags) {
  // recent entries
  const recents = entries.slice(0, RECENT_N)

  // marshal taglist
  const taglist = FIRST_AMONG_EQUALS
    .map((tag) => [tag, tags[tag].length])
    .filter(([_tag, n]) => n > 1) // eslint-disable-line
    .sort((a, b) => a[1] < b[1] ? 1 : -1)
    .map(([tag, n]) => { return { tag, n } })

  // spit homepage
  const introduction = tags.introduction[0]
  const recentByTag = FIRST_AMONG_EQUALS
    .filter((tag) => !NOT_HOME_TAGS.includes(tag))
    .reduce((acc, tag) => {
      const entries = tags[tag].filter((entry) => !recents.includes(entry))
  ...
  const recentAmongTags = FIRST_AMONG_EQUALS
  ...
  await writeHtml('index', templates.homepage({ config, taglist, introduction, recents, recentAmongTags, featured }))
